...
import './style/ListBoards.css'

function ListBoards({ boards = [], onDeleteBoard, onEditBoard }) {
    ...
    const handleEditClick = (boardId, tecName, e) => {
        e.stopPropagation();
        const newName = prompt('Пойман на ошибке! Но ладно давай по новой: ', tecName)
        if (newName && newName.trim() !== '') {
            onEditBoard(boardId, newName.trim())
        }
    }
    ...
    return (
        <div className="boards-list">
            <nav>
                {boards.map((board) => (
                    <button
                        key={board.id} className="board-item" onClick={() => handleBoardClick(board)}>
                        {board.name}
                        <div className="but-tools">
                            ...
                            <svg mlns="http://www.w3.org/2000/svg" className="pencil" viewBox="0 0 16 16" onClick={(e) => handleEditClick(board.id, board.name, e)}>
